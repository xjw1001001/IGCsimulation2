...
def read_rst(filename,filename1,filename2):
    ...
    for line in lines:  
        if len(line)>= 5:
            if lines_index >= tree1_flag1+4 and lines_index <= tree1_flag2-4:
                f1.write(line)
                f1.write('\n')
            if lines_index >= tree2_flag1+4 and lines_index <= tree2_flag2-4:
                f2.write(line)
                f2.write('\n')
        lines_index = lines_index + 1
    f.close()
...
for tau in tau_list:
    for IGCgeo in IGC_geo_list:
        for sim in range(30):
            ...
            f1 = open('/Users/xjw1001001/Documents/GitHub/IGCsimulation2/test/Ancestral_reconstruction/series/YDR418W_YEL054C/'
